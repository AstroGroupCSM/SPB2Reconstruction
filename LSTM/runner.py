...
class Runner(object):
    ...
    def run_one_epoch(self, epoch, mode):
        ...
        for batch_idx, batch_data in enumerate(dataset):
            ...
            batch_outputs = self.model(batch_x, batch_y, x_raw=batch_x_raw)
        ...
        if len(all_labels) > 0 and not mode == 'train':
            ...
            with open(pred_file, 'w+') as f:
                for rec_logits, lbl in zip(all_logits, all_labels):
                    f.write('{},{}\n'.format(','.join([str(l) for l in rec_logits]), lbl))
